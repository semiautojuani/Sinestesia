...
class Ball3D {
    ...
    // Referencias
    PApplet p; // Variable de instancia PApplet
    Ball3D[] others;
    ...
    // Colisión con el mouse: IGNORA EL EJE Z
    void collideWithMouse(PApplet p, PVector mousePos) {
        
        final float REPULSION_FACTOR = PApplet.abs(0.2f);
        
        float mouseX = mousePos.x;
        float mouseY = mousePos.y;
        // float mouseZ = mousePos.z; <-- Ignorado

        // Distancia 3D, forzando la componente Z a 0 (bidimensional)
        float dx = mouseX - this.x;
        float dy = mouseY - this.y;
        float dz = 0; // <--- CLAVE: Ignorar la profundidad en la colisión
        
        float distance = PApplet.sqrt(dx * dx + dy * dy + dz * dz);
        
        // Distancia mínima para que haya repulsión
        ...
        if (distance < minDist && distance > 0) {
            ...
            float nx = dx / distance;
            float ny = dy / distance;
            float nz = 0; // <--- CLAVE: Fuerza de repulsión solo en X e Y
            ...
            this.x -= nx * (minDist - distance) * 0.5f;
            this.y -= ny * (minDist - distance) * 0.5f;
            this.z -= nz * (minDist - distance) * 0.5f; // Esto se resuelve a 0
        }
    }
    ...
    void move(PApplet p, float gravity, float friction, float halfDepth) {
        ...
        // 1. APLICAR ACELERACIÓN / FUERZAS
        this.vy += gravity;

        // 2. APLICAR VELOCIDAD (Movimiento)
        this.x += this.vx;
        this.y += this.vy;
        this.z += this.vz;
    ...
    void display(PShape model, float halfDepthLimit) { 
        ...
        // IMPLEMENTACIÓN DE NIEBLA (FOG) / PROFUNDIDAD
        float zDepthLimit = halfDepthLimit; 
        
        // 1. Efecto de Brillo/Oscurecimiento (Más cerca = Más brillante)
        float depthFactor = PApplet.map(this.z, -zDepthLimit, zDepthLimit, 100, 50);

        // Atenúa el brillo total de la partícula
        brightness *= depthFactor / 100.0f; 

        // 2. Efecto de Transparencia (Más lejos = Más transparente)
        float depthAlpha = PApplet.map(this.z, -zDepthLimit, zDepthLimit, 100, 40);
        ...
        if (model != null) {
            ...
            // Dibujar esfera si no hay modelo
            p.fill(this.hue, saturation, brightness, depthAlpha);
            p.noStroke();
    ...
    // Simulación de getOSCData (retorna un String o PVector)
    PVector getPosition() {
